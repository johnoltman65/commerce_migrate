...
class ProductVariationDeriverTest extends Commerce1TestBase {
  ...
  public function testProductVariationMigrations() {
    ...
    $migrations = $this->pluginManager->createInstances(['d7_commerce_product_variation']);

    // Test that the variation for drinks exists.
    $this->assertArrayHasKey('d7_commerce_product_variation:drinks', $migrations, "Commerce product variation migrations exist after commerce_product installed");

    // Test that the fields for shoes exist in the show migration.
    /** @var \Drupal\migrate\Plugin\migration $migration */
    $migration = $migrations['d7_commerce_product_variation:shoes'];
    ...
    $migration = $migrations['d7_commerce_product_variation:drinks'];
    $process = $migration->getProcess();
